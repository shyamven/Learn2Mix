...
def run_iteration(iter_idx, epochs, chosen_method, return_dict):
    ...
    # ----------------------------------------------------------------
    # Load MNIST dataset with transform.
    ...
    for img, label in fashion_mnist_train:
    ...
    for img, label in fashion_mnist_test:
    ...
    for data_img, label in fashion_mnist_train:
    ...
    if chosen_method == "L2M":
        ...
        X_smote_tensor = torch.tensor(X_smote, dtype=torch.float32).view(-1, 1, 28, 28)
...
if __name__ == '__main__':
    ...
    if chosen_method == "L2M":
        ...
        np.savetxt('train_errors_fashion_mnist.csv', train_errors, delimiter=',')
        np.savetxt('test_errors_fashion_mnist.csv', test_errors, delimiter=',')
        np.savetxt('time_l2m_fashion_mnist.csv', time_l2m, delimiter=',')
    elif chosen_method == "CBL":
        for iters_ in range(iterations):
            balanced_errors[:, iters_], balanced_test_errors[:, iters_], time_cbl[:, iters_] = return_dict[iters_]
        np.savetxt('balanced_errors_fashion_mnist.csv', balanced_errors, delimiter=',')
        np.savetxt('balanced_test_errors_fashion_mnist.csv', balanced_test_errors, delimiter=',')
        np.savetxt('time_cbl_fashion_mnist.csv', time_cbl, delimiter=',')
    elif chosen_method == "SMOTE":
        for iters_ in range(iterations):
            smote_errors[:, iters_], smote_test_errors[:, iters_], time_smote[:, iters_] = return_dict[iters_]
        np.savetxt('smote_errors_fashion_mnist.csv', smote_errors, delimiter=',')
        np.savetxt('smote_test_errors_fashion_mnist.csv', smote_test_errors, delimiter=',')
        np.savetxt('time_smote_fashion_mnist.csv', time_smote, delimiter=',')
    elif chosen_method == "IS":
        for iters_ in range(iterations):
            is_train_errors[:, iters_], is_test_errors[:, iters_], time_is[:, iters_] = return_dict[iters_]
        np.savetxt('is_train_errors_fashion_mnist.csv', is_train_errors, delimiter=',')
        np.savetxt('is_test_errors_fashion_mnist.csv', is_test_errors, delimiter=',')
        np.savetxt('time_is_fashion_mnist.csv', time_is, delimiter=',')
    elif chosen_method == "CL":
        for iters_ in range(iterations):
            cl_train_errors[:, iters_], cl_test_errors[:, iters_], time_cl[:, iters_] = return_dict[iters_]
        np.savetxt('cl_train_errors_fashion_mnist.csv', cl_train_errors, delimiter=',')
        np.savetxt('cl_test_errors_fashion_mnist.csv', cl_test_errors, delimiter=',')
        np.savetxt('time_cl_fashion_mnist.csv', time_cl, delimiter=',')
    elif chosen_method == "classical":
        for iters_ in range(iterations):
            classical_errors[:, iters_], classical_test_errors[:, iters_], time_classical[:, iters_] = return_dict[iters_]
        np.savetxt('classical_errors_fashion_mnist.csv', classical_errors, delimiter=',')
        np.savetxt('classical_test_errors_fashion_mnist.csv', classical_test_errors, delimiter=',')
        np.savetxt('time_classical_fashion_mnist.csv', time_classical, delimiter=',')